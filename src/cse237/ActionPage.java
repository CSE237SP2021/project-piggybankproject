...
public class ActionPage {
	...
	public static void main(String[] args) throws IOException {
		...
		// Continue taking actions until the user types 'exit' to finish
		String userInput = "";
	...
	/**
	 * Checks that user input is a number between 1-7, which covers all given
	 * options in our menu.
	 * 
	 * @param userInput
	 * @throws IOException
	...
	private static void deposit() throws IOException {
		...
		sessionLog.add("Deposited $" + depositAmount + " into account " + accountToUse.getAccountNum() + " - balance: $"
				+ accountToUse.getBalance());
	}
	...
	private static void withdraw() throws IOException {
		...
		if (successfulWithdrawal == false) {
			...
			System.out.println(
					"You have successfully withdrawn $" + withdrawAmount + " from " + accountToUse.getAccountNum());
			sessionLog.add("Withdrew $" + withdrawAmount + " from account " + accountToUse.getAccountNum()
					+ " - balance: $" + accountToUse.getBalance());
		}
	}

	/**
	 * Prints out current user balance.
	 * 
	 * @throws IOException
	 */
	private static void viewBalance() throws IOException {
		Account accountToUse = actionOnOtherAccount();
		accountToUse.setBalance();
		System.out.println("Your current account balance on account " + accountToUse.getAccountNum() + " is: $"
				+ accountToUse.getBalance());
		sessionLog.add("checked balance on account " + accountToUse.getAccountNum() + " - balance: $"
				+ accountToUse.getBalance());
	}
	...
	private static boolean transferMoney() throws IOException {
		...
		// Checks to see if transfer was successful. If not successful, asks for new
		// input.
		while (!success) {
	...
	/**
	 * Checks to make sure that user is able to add another user to their account, and if so, calls the userRepo function to do so. 
	 *
	 * 
	 * @return
	 * @throws IOException
	...
	private static boolean addApprovedUser() throws IOException {
		System.out.println(
				"Please enter the username of the person you would like to add to your account. Please note you may only do this from your main account.");
	...
	/**
	 * Asks user for a new password and updates it in the .txt files.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Retrieves and prints log for current user session in the same run of the
	 * program.
	 */
	 ...
	 * @return the account that a user wants to use in their transactions
	 * @throws IOException
	 */
	private static Account actionOnOtherAccount() throws IOException {
		System.out.println(
				"Please type 0 if you'd like to do this transaction on your main account or 1 if you'd like to do it on an account that you are a subUser");
